...
Now, our model implies that the population mean of $y$ is 
$$E(y) = \alpha + \beta E(x)$$

However, if $y$ is in deviation-from-mean form, its mean has to be zero (since for a random sample the average of $y_i - \bar{y}$ is $E(y)-E(\bar{y})=0$), and similarly for $x$)

In this case, we can drop the constant $\alpha$, so the model is just $y_i = \beta x_i + \varepsilon_i$
...
To find the constant term, recall that $E(y) = \alpha + \beta E(x)$

Thus, we can estimate $\alpha$ as
