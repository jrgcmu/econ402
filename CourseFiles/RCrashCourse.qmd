...
Instead, you should create an **R Script** (a file that ends with .R) in RStudio (`File > New file > R script`), that contains a list of commands that you want to run. This will give you a reproducible record of everything you did
...
We will frequently use the "Tidyverse" packages, which make it a little easier to work in R
...
Before we do anything, we need to change the directory to the location of our file. For example, if it's saved on the Desktop, we can use
...
On windows, we would type

```{r, eval=FALSE}
setwd("C:/Users/[Your Username]/Desktop")
```

Note that we still use the forward slash ``/`` even though Windows usually uses the backslash ``\`` (you can also use ``\\``)
