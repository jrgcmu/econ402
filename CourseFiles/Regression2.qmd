...
First, let's use the fact that $y_i = \beta x_i + \varepsilon$ (plus a few sum properties) to rewrite $b$:
...
## Deriving Gauss-Markov

Suppose $d$ is a *different* linear, unbiased estimator. Then $d$ can be written as

$$
\begin{aligned}
d &= \left(\frac{x_1}{\sum_i x_i^2} + z_1\right) y_1 + \dots + \left(\frac{x_n}{\sum_i x_i^2} + z_n \right) y_n \\
&= \sum_i \left( \frac{x_i}{\sum_i x_i ^2} + z_i \right) y_i \\
&= b + \sum_i z_i y_i
\end{aligned}
$$
...
If $d$ is unbiased, then $E(d)=\beta$, or

$$
\begin{aligned}
E\left( b + \sum_i z_i y_i \right) &= E\left( b + \sum_i z_i (\beta x_i + \varepsilon_i)  \right) \\
&= \beta + \beta \sum_i z_i x_i + \sum_i z_i \underbrace{E(\varepsilon_i)}_{=0} \\
&= \beta
\end{aligned}
$$

which requires that $\sum_i z_i x_i = 0$
...
$$
\begin{aligned}
V(d) &= V\left[ \sum_i \left( \frac{x_i}{\sum_i x_i^2} + z_i \right) y_i \right] = \sum_i V\left[\left( \frac{x_i}{\sum_i x_i^2} + z_i \right) y_i \right] \\
&= \sum_i \left( \frac{x_i}{\sum_i x_i^2} + z_i \right)^2 V(y_i) 
  = \sigma^2 \sum_i \left( \frac{x_i}{\sum_i x_i^2} + z_i \right)^2 \\
&= \sigma^2 \left[\sum_i \frac{x_i^2}{(\sum_i x_i^2)^2}   
  + 2 \sum_i \frac{x_i z_i}{(\sum_i x_i^2)^2} 
  + \sum_i \frac{z_i^2}{(\sum_i x_i^2)^2}\right] \\
&= \sigma^2 \left[ \frac{1}{\sum_i x_i^2} + 0 + \ge 0 \right]
\end{aligned}
$$

---

$V(y_i) = V(\beta x_i + \varepsilon_i) = V(\varepsilon_i)$, since we are treating the $x$s as constants

The middle term is zero because unbiasedness requires that $\sum_i x_i z_i = 0$ and the second term is $\ge$ zero because it is squared
...
We interpreted this as a *relationship* between $x$ and $\widehat y$: for every additional year of schooling, the predicted  wage will increase by 33.5 cents
