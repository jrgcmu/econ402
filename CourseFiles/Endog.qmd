...
Now, we'll investigate the consequences of failures of this assumption
...
If $x_2$ is correlated with $x_1$, then $\varepsilon$ will be as well
...
Intuitively, this is because our prediction $\widehat{x}$ is based on an estimate of the relationship between $x$ and $z$ and not the *true* relationship, so in small samples, $\widehat{x}$ is not completely uncorrelated with $\varepsilon$
