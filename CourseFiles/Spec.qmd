...
As we'll see, we can use econometric specification techniques to learn even more from our regrssions
...
To summarize, the coefficient on the dummy tells us the difference in preditions for those where the dummy is true relative to those where it isn't
...
$$
\begin{aligned}
\widehat{wage}|_{south=1}& = a + b_1 \cdot educ + b2 + b_2 \cdot ecuc \\
&= (a + b_2) + (b_1 + b_2) educ
\end{aligned}
$$
...
In this case, we can control for these time-invariance components by including dummy variables for each individual in our sample:
...
* Each additional year of education increases the probability of being n the labor force by 5%
...
We can use **quadratics** to capture these kinds of non-linear relationshps using a regression
